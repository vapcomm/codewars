...
class RainfallTest {
    ...
    @Test
    fun unknownTown() {
        assertFuzzyEquals(mean("Yerevan", data), -1.0)
        assertFuzzyEquals(variance("Yerevan", data), -1.0)

        assertFuzzyEquals(mean("Lon", data), -1.0)
        assertFuzzyEquals(variance("Lon", data), -1.0)
    }

    @Test
    fun test1() {
        println("Fixed Tests: mean data")
        assertFuzzyEquals(mean("London", data), 51.199999999999996)
    }

    @Test
    fun test2() {
        println("Fixed Tests: variance data")
